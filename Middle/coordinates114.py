...
def coordinates114_Stem_043():
    ...
    comment = "(해설)\n점 $$수식$$A$$/수식$$의 $$수식$$x$$/수식$$좌표를 $$수식$$a$$/수식$$라 하면 점 $$수식$$A$$/수식$$는 $$수식$$y={a}x$$/수식$$의 그래프 위의 점이므로, 점 $$수식$$A$$/수식$$의 좌표는 $$수식$$(a,{a}a)$$/수식$$\n" \
              "(선분 $$수식$$AB$$/수식$$의 길이)$$수식$$={n}$$/수식$$에서 점 $$수식$$B$$/수식$$의 $$수식$$y$$/수식$$좌표는\n" \
              "$$수식$${a}a-{n}$$/수식$$ 이므로 $$수식$$B(a,{a}a-{n})$$/수식$$\n" \
              "같은 방법으로,  $$수식$$C(a+{n},{a}a-{n})$$/수식$$ 이고, $$수식$$D(a+{n},{a}a)$$/수식$$\n" \              
              "이때 점 $$수식$$C$$/수식$$는 $$수식$$y={b}x$$/수식$$의 위의 점이므로\n" \
              "$$수식$$x=a+{n}, y={a}a-{n}$$/수식$$을 대입하면\n" \
              "$$수식$${a}a-{n}={b}(a+{n}) \Rightarrow {s1}a-{s2}={s3}(a+{n})  $$/수식$$"\
              "$$수식$$ \Rightarrow {s4}a={s5} ``````THEREFORE a={s6}$$/수식$$\n" \
              "따라서 점 $$수식$$D$$/수식$$의 좌표는 $$수식$$D{result}$$/수식$$\n"
