...
def generate():
    ...
    if mode == 0 :
        stem = '%s \\times \\left( \\phantom{0}\\phantom{0} \\right) = %s' % (f2, f1)
    elif mode == 1 :
        stem = '\\left( \\phantom{0}\\phantom{0} \\right) \\times %s = %s' % (f2, f1)
    elif mode == 2 :
        stem = '%s = %s \\times \\left( \\phantom{0}\\phantom{0} \\right)' % (f1, f2)
    else :
        stem = '%s = \\left( \\phantom{0}\\phantom{0} \\right) \\times %s' % (f1, f2)
    answer = '%s' % (f6)
    comment = '\\left( \\phantom{0}\\phantom{0} \\right) = %s \\div %s = %s \\times %s = %s%s' % (f1, f2, f1, f3, f5, c1)
