...
def generate():
    while True :
        A, D, F  = np.random.randint(1, 21, 3)
        B, C, E  = np.random.randint(1, 21, 3)

        if is_gcd(A, B) and is_gcd(C, D) and is_gcd(E, F) and max(A//B, C//D, E//F) < 5 :
    ...
    frac_2 = get_fraction(0, D, C, True)
    frac_3 = get_fraction(0, F, E, True)
    ...
    stem = '%s \\div %s \\div %s = ' % (frac_1, frac_2, frac_3)
    answer = '%s' % (frac_5)
    comment = '\\require{cancel} \\\\' \
              '%s \\div %s \\div %s = %s = %s%s' % (frac_1, frac_2, frac_3, c1, frac_4, c2)
