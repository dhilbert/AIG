...
def generate():
    while True :
        ...
        if is_gcd(A, B) and is_gcd(C, D) and is_gcd(E, F) and A != B and C != D and E != F and \
            ...
            E1 = E * (BDF // F)
            G = A1 - C1 - E1
            if BDF < 51 and 0 < G < 51 and G // BDF < 10 and G % BDF and is_gcd(G%BDF, BDF) and max(A1, C1, E1) < 101 :
                break
    ...
    frac_10 = get_fraction(0, G, BDF)
    frac_11 = get_fraction(0, G, BDF, True)
    c2 = ' = %s' % (frac_11) if frac_10 != frac_11 else ''

    stem = '%s - %s - %s = ' % (frac_1, frac_2, frac_3)
    answer = '%s' % (frac_11)
    comment = '%s - %s - %s %s= %s - %s - %s = %s%s' % (frac_1, frac_2, frac_3, c1, frac_7, frac_8, frac_9, frac_10, c2)
